...
// Fallback image for when media fails to load
...
export const ImageMedia: React.FC<MediaProps> = (props) => {
  ...
  // Use fallback image if there's an error and we've exhausted retries
  const imageSrc = imageError && retryCount >= 2 ? fallbackImage : src
  ...
  return (
    <picture className={cn(pictureClassName)}>
      <NextImage
        ...
        priority={priority}
        quality={imageError ? 75 : 100} // Reduce quality for fallback
        loading={loading}
        sizes={sizes}
        src={imageSrc}
        ...
        unoptimized={imageError || process.env.NODE_ENV === 'development'}
      />
    </picture>
  )
}
