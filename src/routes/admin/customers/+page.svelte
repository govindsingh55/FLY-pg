...
<div class="flex h-full flex-col gap-4 p-6">
	...
	<div class="flex items-center gap-2 flex-wrap">
		<InputGroup.Root class="max-w-sm">
			<InputGroup.Input placeholder="Search customers..." bind:value={searchTerm} />
			<InputGroup.Button>
		...
		<svelte:boundary>
			...
			{:catch error}
				<div class="p-8 text-center text-destructive">
					...
					<Button variant="outline" onclick={() => (dataPromise = getCustomers({}))} class="mt-2">
						Try Again
					</Button>
				</div>
			{/await}
...
<CustomerForm bind:open={isCreateOpen} />
